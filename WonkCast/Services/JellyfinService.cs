...
namespace WonkCast.Services
{
    public class JellyfinService
    {
        ...
        private readonly Func<string, string, Task<string>> AccessTokenDelegate;
        ...
        public JellyfinService(AccessTokenCaller<JellyfinServiceAppProvider> jellyfinCaller, AccessTokenCaller<JellyfinAuthenticationServiceAppProvider> jellyfinAuthCaller, IConfiguration configuration)
        {
            ...
            AccessTokenDelegate = async (string credential, string code) =>
            {
                ...
                Dictionary<string, string> headerReplacements = new() { { "$Device", "WonkCast" }, { "$DeviceId", "WonkCast" } };
        ...
        public async Task<List<Media>> GetItems(JellyPhrase phrase, string userId, string device = "WonkCast", string deviceId = "WonkCast")
        {
            ...
            CallResult<string> foldersCallResult = await JellyfinCaller.GetRequestAsync<string>($"Users/{userId}/Items", phrase.User, AccessTokenDelegate, queryParameters, headerReplacements);
            ...
            foreach (Item folder in JsonConvert.DeserializeObject<JellyfinItems>(foldersCallResult.Content ?? string.Empty)?.Items ?? Array.Empty<Item>())
            {
                ...
                recursiveTasks.Add(Task.Run(async () =>
                {
                    ...
                    CallResult<string> childFoldersCallResult = await JellyfinCaller.GetRequestAsync<string>($"Users/{userId}/Items", phrase.User, AccessTokenDelegate, queryParameters, headerReplacements);
        ...
        public async Task<string?> GetUserId(string userName, string device = "WonkCast", string deviceId = "WonkCast")
        {
            Dictionary<string, string> headerReplacements = new() { { "$Device", device }, { "$DeviceId", deviceId } };
        ...
        private async Task<IEnumerable<Item>> GetItems(JellyPhrase phrase, string userId, string? parentId, Dictionary<string, string> headerReplacements)
        {
            ...
            CallResult<string> callResult = await JellyfinCaller.GetRequestAsync<string>($"Users/{userId}/Items", phrase.User, AccessTokenDelegate, queryParameters, headerReplacements);
        ...
        private List<Media> ItemsToMediaQueue(IEnumerable<Item> items, JellyPhrase phrase, string userId)
        {
            return items.Select((item, index) => 
            {
                ...
                if (!mediaKind.HasValue)
                    ...
                    return new Media
                    {
                        ...
                        Metadata = GetMetadata((MediaKind)mediaKind, item, userId),
